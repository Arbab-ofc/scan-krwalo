...
export function AuthNav() {
  ...
  if (!user) {
    return (
      <div className="flex items-center gap-2">
        <Link className="hidden rounded px-3 py-2 font-medium text-slate-600 hover:text-ink sm:block" href="/signup">Signup</Link>
        <Link className="focus-ring flex items-center gap-2 rounded-md bg-accent px-4 py-2 font-medium text-white shadow-glow" href="/login"><LogIn size={16} /> Login</Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2">
      <Link className="focus-ring hidden items-center gap-2 rounded-md border border-line bg-white px-4 py-2 font-medium text-ink shadow-sm sm:flex" href="/dashboard"><LayoutDashboard size={16} /> Dashboard</Link>
