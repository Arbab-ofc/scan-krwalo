        ...
        <section className="app-card">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="app-eyebrow">Payout profile</p>
              <h1 className="app-title">Payment destination</h1>
          ...
          <form action={submit} className="mt-8 grid gap-5">
            ...
            <label className="grid gap-2 text-sm font-medium text-slate-700">
              ...
              <span className="relative">
                ...
                <input
                  ...
                  className="app-input pl-11 font-mono"
            ...
            <div className="flex flex-col gap-3 border-t border-line pt-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button disabled={isPending} className="app-button min-h-12 rounded-xl bg-accent px-6 text-white shadow-glow disabled:cursor-not-allowed disabled:opacity-70">
...
function StatusRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-4 rounded-2xl border border-line px-4 py-3">
      <div className="flex items-center gap-3 text-sm font-medium text-slate-600">
        <span className="text-accent">{icon}</span>
        {label}
      </div>
      <span className="break-safe max-w-[52%] text-right text-sm font-semibold text-ink">{value}</span>
    </div>
  );
}
